...
Usage:
    ...
    uv run python scripts/sync.py --channel bd         # Sync specific channel
    uv run python scripts/sync.py --channel mycompany/general  # Sync channel with workspace prefix
...
def sync_workspace(
    ...
    with SlackClient(xoxc_token, xoxd_token, workspace=subdomain, storage=storage) as client:
        # Resolve channels to sync
        if channels == ["*"]:
            # Wildcard: list all channels from the API
            print(f"  Listing all channels...")
            ...
            # Look up specific channels by name
            resolved_channels = []
            for channel_name in channels:
                try:
                    ...
                    print(f"  Error looking up #{channel_name}: {e}")
...
def main():
    ...
    parser.add_argument(
        "--channel",
        help="Specific channel to sync (e.g., 'general' or 'mycompany/general')",
    )
    parser.add_argument(
        "--no-dms",
        action="store_true",
    ...
    for ws in workspaces:
        channels_to_sync = ws.channels

        # Filter by --channel if specified
        if args.channel:
            channel_filter = args.channel
            # Handle both "channel" and "workspace/channel" formats
            if "/" in channel_filter:
                filter_ws, filter_ch = channel_filter.split("/", 1)
                if filter_ws != ws.name:
                    continue
                channel_filter = filter_ch
            # When wildcarded, the specific channel filter overrides
            if channels_to_sync == ["*"]:
                channels_to_sync = [channel_filter]
            else:
                channels_to_sync = [c for c in ws.channels if c == channel_filter]
