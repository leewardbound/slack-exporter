...
def load_channels_config() -> dict[str, list[str]]:
    ...
    A single entry of "*" means subscribe to all channels in that workspace.
    """
        ...
        for line in channels_path.read_text().splitlines():
            ...
            if "/" in line:
                ...
                if channel == "*":
                    ...
                    workspace_channels[workspace] = ["*"]
                elif "*" not in workspace_channels[workspace]:
                    # Only add specific channels if not already wildcarded
